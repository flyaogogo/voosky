...
public class BakMysql {
	...
	public void backup() {
		try {
			...
			Process child = rt.exec("mysqldump -uroot -p123456 voosky");
			// 设置导出编码为utf8,这里必须是utf8。
			// 注意这一句，是指运行mysqldump命令，后面跟的是登录名和登录的密码，接着后面的是指备份的数据库的名字，到此结束，
			// 以此生成一个执行的进程，取得此进程的输出流到我们要备份的文件
	...
	public String backupMySqlToFile() {
		...
		try {
			Runtime rt = Runtime.getRuntime();
			Process child = rt
					.exec("C:\\Program Files\\MySQL\\MySQL Server 5.5\\bin\\mysqldump -uroot -p123456 voosky");
			// 设置导出编码为utf8。 这里必须是utf8在此要注意 ，
			// 有时会发生一个mysqldump:
	...
	public void restoreMysqlFromFile(File f) {
		try {
			...
			// 这里执行的是mysql命令，用户名，密码以及要恢复的数据库，命令执行完后会从我们上传的文件里面读取要执行的内容
			OutputStream out = child.getOutputStream();
			// 控制台的输入信息作为输出流
			String inStr;
			StringBuffer sb = new StringBuffer("");
